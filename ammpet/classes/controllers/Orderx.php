...
class Orderx
{
    ...
    public function get_packages(){
        if (isset($_GET['cli_id'])){
            ...
            $sql_stm="SELECT CP.ID AS PCT, CP.ID_ORDER AS ID_ORDER, CP.ID_ORDER_ITEM AS ID_ORDER_IT, CP.PACK_QUANTITY, CP.PACK_CONSUMED, CP.PACK_STATUS, GROUP_CONCAT(A.NAME,\" - \", OI.PACKAGE_SERVICE,\" - \", OI.PACKAGE_SEQUENCE,\" - \", OI.ID,\" - \", OI.DATE SEPARATOR '<br>') AS DETAIL FROM CLIENT_PACKAGE CP LEFT JOIN (ORDER_ITEM OI, ANIMAL A) ON (CP.ID=OI.ID_PACKAGE AND CP.ID_ANIMAL=A.ID) WHERE CP.ID_CLIENT=:ID_CLIENT and CP.PACK_STATUS =:PACK_STATUS GROUP BY CP.ID";
